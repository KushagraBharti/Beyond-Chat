...
| Supabase CLI token | Data owner | Supabase CLI credential store | Repo/env files | Rotate on device/collaborator loss | Reauthenticated 2026-07-11 |
...
| Composio legacy full-access key | Integration admin | Admin secret store only | Application runtime | Revoke immediately after scoped-key integration passes | Exists for rollback; must not be wired |
...
| `WORKOS_API_KEY` | Identity owner | Backend Vercel only | Frontend, sandbox | Create after canonical Production activation; quarterly/on exposure | Not created or installed; WorkOS Production billing activation is intentionally deferred |
...
| `STRIPE_SECRET_KEY` | Billing owner | Backend Vercel only | Frontend, runner, Modal | Rotate on exposure; key must match intended account | Intended-account key not installed; local key mismatch |
...
| Modal service token/identity | Runtime owner | Modal/Vercel backend secret store as required | Frontend/sandbox payload | Prefer service identity; rotate on owner change | CLI user auth only; service identity absent |
