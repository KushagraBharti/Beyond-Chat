...
class LegacyLocalDataStore:
    ...
    def upsert_artifact(
        ...
        *,
        ...
        if source_run_id:
            merged_metadata.setdefault("source_run_id", source_run_id)
        if storage_path:
...
legacy_runtime_store = LegacyLocalDataStore()


def get_runtime_store(context: "RequestContext | None" = None) -> RuntimeDataStore:
    if context and context.source == "supabase_jwt" and supabase_service.runtime_database_enabled:
        return SupabaseDataStore(context.access_token, context.user_id)
    return legacy_runtime_store
