...
export const WorkspaceComposer = forwardRef<HTMLTextAreaElement, { disabled: boolean; onPromote: (draft: PromotionDraft) => void; onSend?: (draft: PromotionDraft) => void; onBrowse: (path: string) => void; items?: readonly (typeof discoveryItems)[number][] }>(({ disabled, onPromote, onSend, onBrowse, items = discoveryItems }, forwardedRef) => {
  ...
  return <div className="workspace-composer">
    ...
    <div className="workspace-composer-actions"><span><kbd>/</kbd> discover · <kbd>↑↓</kbd> select · chips keep context explicit</span><span>{onSend ? <button type="button" className="workspace-button" disabled={!message.trim() || disabled} onClick={() => { onSend({ prompt: message.trim(), references: chips, source: "chat" }); setMessage(""); setChips([]); }}>Run General</button> : null}<button type="button" className="workspace-button is-quiet" disabled={!message.trim() || disabled} onClick={() => onPromote({ prompt: message.trim(), references: chips, source: "chat" })}>Promote to Work</button></span></div>
