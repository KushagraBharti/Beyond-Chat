...
export const WorkspaceComposer = forwardRef<HTMLTextAreaElement, { disabled: boolean; onPromote: (draft: PromotionDraft) => void; onBrowse: (path: string) => void }>(({ disabled, onPromote, onBrowse }, forwardedRef) => {
  ...
  return <div className="workspace-composer">
    ...
    <div className="workspace-composer-actions"><span><kbd>/</kbd> discover · <kbd>↑↓</kbd> select · chips keep context explicit</span><button type="button" className="workspace-button" disabled={!message.trim() || disabled} onClick={() => onPromote({ prompt: message.trim(), references: chips, source: "chat" })}>Promote to Work</button></div>
